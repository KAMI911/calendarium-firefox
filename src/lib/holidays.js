/*
 * holidays.js — National holiday loader for Calendarium
 *
 * Ported from the calendarium@kami911 Cinnamon desklet's lib/holidays.js.
 * All date-math/query logic (getHolidayForDate, getHolidaysRange,
 * getPeriodsForDate, getUpcomingPeriods, Easter calculation, period
 * endpoint resolution) is byte-identical; the I/O primitive was swapped
...
export const Holidays = {
     ...
     * @param {string}   dataDir   Extension-relative path to data/holidays (e.g. "data/holidays")
    ...
    loadData: function(dataDir, locale, callback) {
        ...
        var url = browser.runtime.getURL(dataDir + "/" + locale + ".json");
        var promise = fetch(url)
            .then((r) => { if (!r.ok) throw new Error("HTTP " + r.status); return r.json(); })
            .catch((e) => {
                console.error("Calendarium: failed to load holiday data for '" + locale + "': " + e);
                return null;
            });
